...
uses SESSION, Classes, Dialogs, Sysutils, Messages;
...
begin
  ...
  // 1)  Apabila Ada pelajaran yg melebihi jam maximal pelajaran dan bentrok pada ruang yang sama...
  // Dua atau lebih kelas matakuliah yang berbeda tidak dapat berada pada hari, ruang dan jam yang sama.
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Alelle[i].KodeJam + JP1) > (JUMLAHJAM) then
          begin
            Fitness := Fitness + 1;
            PosGenTabrk := i;
          ...
          begin
            // Apabila ada kelas kuliah dalam satu ruangan seblum JP habis..
            if (Kromosom[NoKromosom][i + 1] <> '0') and
              (Alelle[i].KodeRuang = Alelle[i + 1].KodeRuang) then
            begin
              Fitness := Fitness + 1;
              PosGenTabrk := i;
              MatTabrak[Fitness] := Kromosom[NoKromosom][i];
              MatTabrak1[Fitness] := Kromosom[NoKromosom][i + 1];
            end;
          end
          else if JP1 = 3 then
          begin
            // Apabila ada kelas dalam satu ruangan seblum JP habis..
            if (Kromosom[NoKromosom][i + 1] <> '0') and
              (Alelle[i].KodeRuang = Alelle[i + 1].KodeRuang) then
            begin
              Fitness := Fitness + 1;
            ...
            if (Kromosom[NoKromosom][i + 2] <> '0') and
              (Alelle[i].KodeRuang = Alelle[i + 2].KodeRuang) then
            begin
              Fitness := Fitness + 1;
              PosGenTabrk := i;
              MatTabrak[Fitness] := Kromosom[NoKromosom][i];
              MatTabrak1[Fitness] := Kromosom[NoKromosom][i + 2];
            end;
          end
          else if JP1 = 4 then
          begin
            if (Kromosom[NoKromosom][i + 1] <> '0') and
              (Alelle[i].Ruang = Alelle[i + 1].Ruang) then
            begin
              Fitness := Fitness + 1;
              PosGenTabrk := i;
              MatTabrak[Fitness] := Kromosom[NoKromosom][i];
            ...
            if (Kromosom[NoKromosom][i + 2] <> '0') and
              (Alelle[i].Ruang = Alelle[i + 2].Ruang) then
            begin
              Fitness := Fitness + 1;
              PosGenTabrk := i;
              MatTabrak[Fitness] := Kromosom[NoKromosom][i];
            ...
            if (Kromosom[NoKromosom][i + 3] <> '0') and
              (Alelle[i].Ruang = Alelle[i + 3].Ruang) then
            begin
              Fitness := Fitness + 1;
              PosGenTabrk := i;
              MatTabrak[Fitness] := Kromosom[NoKromosom][i];
              MatTabrak1[Fitness] := Kromosom[NoKromosom][i + 3];
            end;
          end;
        end;
      end; // end for i
  ...
  // 2) Hitung MAximum jam dalam satu hari semua ruangan..
  ...
  begin
    ...
    with FrmPenjadwalan.TblKegiatanQuery do
    begin
      for i := 0 to TotalGen - (1 + JedaHari) do
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              KodeMtk1 := FieldByName('mpid').AsInteger;
              ...
              begin
                ...
                // Satu jurusan,kelas,dan semester yang sama mempelajari KULIAH yg beda dlm 1 Hari
                if (KodeMtk1 <> KodeMtk2) and (Semester1 = Semester2) and
                  (Jurusan1 = Jurusan2) and (Kelas1 = Kelas2) then
                begin
                  Fitness := Fitness + 1;
                  PosGenTabrk := i;
                  MatTabrak[Fitness] := Kromosom[NoKromosom][i];
              ...
              else if (JP1 = 2) and (Alelle[i].KodeJam + JP1 <= (JUMLAHJAM + 1)
                ) then
              begin
                if Locate(KEY_FIELD, Kromosom[NoKromosom][i + (j * JUMLAHJAM)
                    + 1], []) then
              ...
              else if (JP1 = 3) and (Alelle[i].KodeJam + JP1 <= (JUMLAHJAM))
                then
              begin
                if Locate(KEY_FIELD, Kromosom[NoKromosom][i + (j * JUMLAHJAM)
                    + 1], []) then
                begin
                  ...
                  Kelas2 := FieldByName('kelasid').AsInteger;
                  KodeMtk2 := FieldByName('mpid').AsInteger;
                  if (Semester1 = Semester2) and (KodeMtk1 <> KodeMtk2) and
                    (Jurusan1 = Jurusan2) and (Kelas1 = Kelas2) then
                  begin
                    Fitness := Fitness + 1;
                    PosGenTabrk := i;
              ...
              if Alelle[i].KodeJam <> 1 then
              begin
                if Kromosom[NoKromosom][i + (j * JUMLAHJAM) - 1] <> '0' then
                begin
                  ...
                  Kelas2 := FieldByName('kelasid').AsInteger;
                  KodeMtk2 := FieldByName('mpid').AsInteger;
                  JP2 := FieldByName(JAM_PERTEMUAN_FIELD).AsInteger;
                  if (JP2 = 2) and (Semester1 = Semester2) and
                    (KodeMtk1 <> KodeMtk2) and (Jurusan1 = Jurusan2) and
                    (Kelas1 = Kelas2) then
                  begin
                    Fitness := Fitness + 1;
                    PosGenTabrk := i;
                ...
                else if (Kromosom[NoKromosom][i + (j * JUMLAHJAM) - 2] <> '0')
                  and (Alelle[i - 2].KodeJam > 0) then
                begin
                  Locate(KEY_FIELD, Kromosom[NoKromosom][i + (j * JUMLAHJAM)
                      - 2], []);
                  Jurusan2 := FieldByName('jid').AsInteger;
                  ...
                  KodeMtk2 := FieldByName('mpid').AsInteger;
                  JP2 := FieldByName(JAM_PERTEMUAN_FIELD).AsInteger;
                  if (JP2 = 3) and (Semester1 = Semester2) and
                    (KodeMtk1 <> KodeMtk2) and (Jurusan1 = Jurusan2) and
                    (Kelas1 = Kelas2) then
                  begin
                    Fitness := Fitness + 1;
          ...
          // Cek Ruangan 1 ke Ruangan 2 dst... max : 60 Ruangan dlm 6 Hari...
  ...
  // 4. Dosen tidak boleh mengajar lebih dari satu kelas dalam jam yang sama
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if i + (j * JUMLAHJAM) >= (IndHari * JedaHari) then
                ShowMessage(inttostr(i) + #13#10 + inttostr(IndRuang));
              // Ambil Record Sesuai kode_kelasMK pertama
              Locate(KEY_FIELD, Kromosom[NoKromosom][i], []);
              Dosen1 := FieldByName('pid').AsInteger;
              JP1 := FieldByName(JAM_PERTEMUAN_FIELD).AsInteger;
              // KodeMtk1 := FieldByName('kode_matapelajaran').AsString;

              // Ambil MataKuliah Pada Jam yg sama tapi ruangan berbeda..
              if Locate(KEY_FIELD, Kromosom[NoKromosom][i + (j * JUMLAHJAM)],
                []) then
              begin
              ...
              else if (JP1 = 2) and (Alelle[i].KodeJam + JP1 <= (JUMLAHJAM))
                then
              begin // Ambl MK Pada Jm yg +- JP pada ruang berbeda
                if Locate(KEY_FIELD, Kromosom[NoKromosom][(i + 1) +
                    (j * JUMLAHJAM)], []) then
                begin
                  Dosen2 := FieldByName('pid').AsInteger;
                  if Dosen1 = Dosen2 then
              ...
              begin
                ...
                begin
                  ...
                  Dosen2 := FieldByName('pid').AsInteger;
                  JP2 := FieldByName(JAM_PERTEMUAN_FIELD).AsInteger;
                  if (JP2 = 2) and (Dosen1 = Dosen2) then
                  begin
                    Fitness := Fitness + 1;
                    PosGenTabrk := i;
                    MatTabrak[Fitness] := Kromosom[NoKromosom][i];
                ...
                begin
                  ...
                  Dosen2 := FieldByName('pid').AsInteger;
                  JP2 := FieldByName(JAM_PERTEMUAN_FIELD).AsInteger;
                  if (JP2 = 3) and (Dosen1 = Dosen2) then
                  begin
                    Fitness := Fitness + 1;
                    PosGenTabrk := i;
                    MatTabrak[Fitness] := Kromosom[NoKromosom][i];
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if Kromosom[NoKromosom][j] <> '0' then
            begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FrmPenjadwalan.ZQuery.Active := False;
          FrmPenjadwalan.ZQuery.SQL.Text :=
            'select count(paid) from pengajar_aturan where pid=:pengajar and hariid=:hari and (jamid>=:jam1 or jamid<=:jam2)';
          FrmPenjadwalan.ZQuery.Params.ParamByName('pengajar').AsInteger :=
            Dosen1;
          FrmPenjadwalan.ZQuery.Params.ParamByName('hari').AsInteger := Alelle
            [i].KodeHari;
          FrmPenjadwalan.ZQuery.Params.ParamByName('jam1').AsInteger := Alelle
            [i].KodeJam;
          FrmPenjadwalan.ZQuery.Params.ParamByName('jam2').AsInteger := Alelle
            [i].KodeJam + JP1;
          FrmPenjadwalan.ZQuery.Open;
          if (FrmPenjadwalan.ZQuery.Fields[0].AsInteger > 0) then
          begin
  ...
  // 8. Penempatan Tingkat (X-XI-XII) di ruangan yang seharusnya
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (BilRandom > 1) and (BilRandom < (TotalGen - 2)) then
        begin
          if (Kromosom[NoKromosom][BilRandom] = '0') and
            (Kromosom[NoKromosom][BilRandom + 1] = '0') and
            (Kromosom[NoKromosom][BilRandom + 2] = '0') and
            (Kromosom[NoKromosom][BilRandom - 1] = '0') and
            (Kromosom[NoKromosom][BilRandom - 2] = '0') and
        ...
        else if (BilRandom = (0 * JedaHari)) Or (BilRandom = (1 * JedaHari)) Or
        ...
        else if (BilRandom = ((1 * JedaHari) - 2)) Or
          (BilRandom = ((2 * JedaHari) - 2)) Or
          (BilRandom = ((3 * JedaHari) - 2)) Or
          (BilRandom = ((4 * JedaHari) - 2)) Or
          (BilRandom = ((5 * JedaHari) - 2)) Or
          (BilRandom = ((6 * JedaHari) - 2)) then
        begin
          if (Kromosom[NoKromosom][BilRandom] = '0') and
            (Kromosom[NoKromosom][BilRandom - 1] = '0') and
            (Kromosom[NoKromosom][BilRandom - 2] = '0') then
